...
export class viewHistoryCommand extends BaseCommand {
  async run() {
    ...
    if (EditorState.connection) {
      ...
      const selectedConnection = await vscode.window.showQuickPick(
        quickPicks,
        { placeHolder: 'Select a connection to view its history' }
      );
    ...
    // 调用 SQLHistory 实例的方法来显示历史记录
    await SQLHistory.getInstance().viewHistory(connection);
  }
}
